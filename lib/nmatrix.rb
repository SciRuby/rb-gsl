...
class NMatrix
  ...
  class << self
    ...
    def cblas_gemm a, b, c=nil, alpha=1.0, beta=0.0, trans_a=false, trans_b=false, m=nil, n=nil, k=nil, lda=nil, ldb=nil, ldc=nil
      ...
      # First, set m, n, and k, which depend on whether we're taking the transform of a and b.
      if c.nil?
        if trans_a # either :transform or :complex_conjugate
          m ||= a.shape[1]
          k ||= a.shape[0]
        else # no transform
          m ||= a.shape[0]
      ...
      # I think these are independent of whether or not a transform occurs.
      lda ||= a.shape[1]
      ldb ||= b.shape[1]
      ldc ||= c.shape[1]

      # For argument descriptions, see: http://www.netlib.org/blas/dgemm.f
      NMatrix.__cblas_gemm__(trans_a, trans_b, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
