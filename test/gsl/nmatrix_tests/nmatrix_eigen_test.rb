...
class NMatrixEigenTest < GSL::TestCase
  ...
  def test_symm_symmv
    ...
    assert_rel GSL::Eigen.symm(@nmatrix), eigen_values, 0.001, "GSL::Eigen.symm(nmatrix)"

    val, vec = GSL::Eigen.symmv(@nmatrix)

    assert_rel val, eigen_values , 0.001, "GSL::Eigen.symmv(nmatrix)"
    assert_rel val, eigen_vectors, 0.001, "GSL::Eigen.symmv(nmatrix)"
  end
end
